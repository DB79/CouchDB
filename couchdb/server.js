...
var dbName = "players";
var targetOfReplication = "playersrepl";
var port = 3000;
...
db.exists((err, exists)=>{
    ...
    else if (exists) 
    {
      ...
      //saves a design document named viewAll with two views, on simple view and one with map reduce
      db.save('_design/viewAll', {
          //create view named all
          all: {
                map: function(doc) {
                    emit(doc._id,doc);  
                }
                 
          },//create view named allWithTrophies with map reduce
          allWithTrophies: {
              map: function (doc) {

                key = {id: doc._id, username: doc.username, level: doc.level}
                if(typeof doc.trophy != "undefined"){
                (doc.trophy || []).forEach(function(trophy){
                                        emit(key, 1);
                });
                }
                else{
...
replRoute.post((req,res)=>{
    ...
    console.log('In replication method' + targetOfReplication)
